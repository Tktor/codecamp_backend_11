...
describe('UsersService', () => {
  ...
  //   expect(result).toStrictEqual({ // 객체 비교 시에 사용
  //     email: 'a@a.com',
  //     name: '짱구',
  //     ...
  //   })
  ...
  describe('create', () => {
    it('이미 존재하는 이메일 검증하기!!', async () => {
      ...
      try {
        ...
        // expect(error).toBeInstanceOf(UnprocessableEntityException); // 잘 작동하는지 확인용
      }
    });

    it('회원 등록 잘 되었는지 검증하기!!', async () => {
      const myData = {
        email: 'bbb@bbb.com',
        password: '1234',
        name: '철수',
  ...
  // TDD => 테스트를 먼저 만들자!
  it('이메일 길이가 초과되었을때 검증!!!', async () => {
    const myData = {
      email: 'bbbadasdasdasdasdasdas@bbb.com',
      password: '1234',
      name: '철수',
      age: 13,
    };
